...
## 1章　なぜTerraformを使うのか
- イミュータブル(変更不可能)なインフラをサポート
    - Terraformはインフラの状態を宣言的に定義する。変更がある場合、Terraformは必要なリソースを破棄してから新しいリソースを作り直す、またはリソースの更新を行う。これにより、常に一貫したインフラ状態を保証し、予期しない変更がインフラの一部だけに発生するリスクが少なくなる。ミュータブルなツールは、既存のサーバーやインフラに変更を加えるため、インフラの状態が部分的に更新される。時間が経つと、サーバーごとに微妙な違いが蓄積される「構成ドリフト」が発生するリスクがある。
- 宣言型言語
    - 「何を実現したいか」に焦点を当て、処理の結果だけを指定するプログラミング。インフラの望ましい状態を定義。手続き型言語の大きなデメリットとして、処理の順番を知る必要がある
- マスタレス
    - 分散システムにおいて特定の「マスタ」ノードが存在せず、すべてのノードが対等に振る舞うアーキテクチャ。マスタと比較して、余計なインフラ、メンテナンス、セキュリティといった追加の管理が不要。
- エージェントレス
    - エージェント
        - システム管理やアプリケーション運用において、特定のタスクを実行したり、管理対象のシステムを操作・監視するために管理対象のデバイスやサーバーにインストールされるソフトウェアコンポーネントのこと。
    - エージェントレスのTerraformはインフラの操作がリモートAPIを通じて行われる。Terraformはクラウドプロバイダ（AWS、Azure、Google Cloudなど）やオンプレミスのインフラにアクセスする際、各プロバイダの公開されたAPIを直接使用する。このため、管理対象のサーバーにエージェントをインストールする必要が無く、バグの原因の切り分けが必要無い。
- プロビジョニングツール
    - プロビジョニング(必要なものを準備、提供すること)
    - クラウドやオンプレミスのインフラ全体を一貫してデプロイ・管理できる。サーバー、ネットワーク、ストレージ、ロードバランサー、データベースなど、インフラ全体のリソースをコード化し、管理・自動化できる。クラウドリソースだけでなく、Kubernetesクラスタやネットワーク設定など、広範なリソースを統合的に扱える
    - 設定管理ツール
        - 主にサーバー上のソフトウェアや設定ファイルの管理に特化しており、既に存在しているサーバーやリソースの設定を変更・管理するツール。これらはサーバーの設定や構成管理に強みがあるが、インフラ全体のプロビジョニングやリソースの作成には特化していない。
## 2章　Terraformをはじめよう
